...
def get_or_create_project_type(conn, description):
    try:
        with conn.cursor() as cursor:
            logger.info(
                f"Attempting to retrieve project type: '{description}' with company ID: '{company_id}'")
            cursor.execute(get_project_type, (description, company_id))
            result = cursor.fetchone()

            if result:
                project_type_key = result[0]
                logger.info(
                    f"Project type '{description}' found with key: '{project_type_key}'")
                return project_type_key, False
            else:
                logger.info(
                    f"Project type '{description}' not found, creating new entry.")
                ...
                logger.info(
                    f"Created new project type '{description}' with key: '{project_type_key}'")
                return project_type_key, True

    except psycopg2.Error as e:
        logger.error(
            f"Database error while processing project type '{description}': {e}")
        raise
