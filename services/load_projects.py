...
from config import CLIENT_ID, CREATED_BY, CREATED_AT, CITY, STATE, STATE_ID, PROJECT_TYPE, PROJECT_CODE_COUNTER
...
def load_projects(conn):
    ...
    for project_name, project_status, no_of_units, project_address in projects:
        ...
        project_key, created = get_or_create_project(
            conn, project_name, project_status_key, no_of_units, project_address, state_key, city_key, project_type_key, project_code
        )
        if created:
        ...
        logger.info(
            f"The key of the project '{project_name}' is: '{project_key}', Project Code: '{project_code}'")
